...
async function buildReportLines(
  ...
  return rows.map((row) => ({
    ...
    notaFiscal:
      (invoiceDocumentsByPurchaseId.get(row.id) ?? [])
        .map((item) => item.documentNumber ?? '(sem nº)')
        .join(' / ') || '—',
...
async function buildExcelResponse(lines: ReportLine[]) {
  ...
  sheet.columns = [
    ...
    { header: 'Código de Lançamento', key: 'ordemCompra', width: 20 },
...
// A4 paisagem para caber a coluna extra de fornecedor (distinto do estabelecimento em
...
const COLUMN_X: Record<ColumnKey, number> = {
  ...
  estabelecimento: MARGIN + 150,
  fornecedor: MARGIN + 280,
  requisicao: MARGIN + 400,
  ordemCompra: MARGIN + 450,
  centroCusto: MARGIN + 500,
  valor: MARGIN + 570,
  status: MARGIN + 615,
};
